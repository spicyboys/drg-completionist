...
export const getArmorPaintJobsFromSaveFile = ({CharacterSaves}: SaveFile): ArmorPaintjobEntry[] => {
  ...
  const idToEntry = Object.fromEntries(Object.entries(ArmorPaintjobs).flatMap(([miner, paintJobs]) => {
    return paintJobs.map((paintJob) => [paintJob.partID, {
      miner,
      name: paintJob.name,
    } as ArmorPaintjobEntry]);
  }));

  for (const {Vanity: {CharacterVanitySave: { UnLockedVanityItemIDs: ids }}} of CharacterSaves) {
    for (const id of ids) {
      ...
      if (entry != null && !acquiredArmorPaintjobs.includes(entry)) {
        acquiredArmorPaintjobs.push(idToEntry[id]);
      }
    }
  }
